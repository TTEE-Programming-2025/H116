...
// 心得：
// 這個程式的設計過程，我學到如何處理使用者輸入、隨機數字生成以及座位排程的邏輯。
// 在這個作業，要學會如何在C語言中使用陣列來儲存座位資料，還要學會如何處理密碼檢查、座位預約以及隨機選擇座位等功能。
// 這個作業有助於加深對流程控制的理解和處理使用者互動與隨機生成結果的部分，讓我需要更靈活地應用程式設計的技巧。
// 也讓我再次練習如何以簡潔易懂的方式設計界面，讓使用者能夠順利操作。
// 這個程式是個比前面兩個難的新挑戰 。 
